...
(function () {
      ...
      actions: [
        {
          ...
          icon: "🔎",
          run: () => { window.location.href = "all-martyrs/all-martyrs.html"; }
        },
        {
          title: "افزودن نام تکی",
          description: "افزودن یک نام به اطلاعات شهدا",
          icon: "＋",
          run: () => { window.location.href = "all-martyrs/all-martyrs.html#single"; }
        },
        {
          title: "افزودن نام گروهی",
          description: "افزودن گروهی اطلاعات شهدا",
          icon: "▦",
          run: () => { window.location.href = "all-martyrs/all-martyrs.html#bulk"; }
        }
      ]
    },
    {
  ...
  function installStyles() {
    ...
    style.textContent = `
      ...
      .modular-home-back{color:#7b4a22}.modular-home-refresh{color:#17633d}
      ...
      @media(max-width:760px){.modular-home-card{padding:14px;border-radius:17px}.modular-home-card-title{font-size:18px}.modular-home-actions{grid-template-columns:1fr}.modular-home-action{min-height:60px}}
    `;
  ...
  function buildHome() {
    ...
    let scrollTop = document.getElementById("modular-home-scroll-top");
    if (!scrollTop) {
      scrollTop = document.createElement("button");
      scrollTop.id = "modular-home-scroll-top";
    ...
    if (!window.__modularHomeScrollHandler) {
      window.__modularHomeScrollHandler = () => {
        const button = document.getElementById("modular-home-scroll-top");
        if (button) button.classList.toggle("visible", window.scrollY > 280);
      };
      window.addEventListener("scroll", window.__modularHomeScrollHandler, { passive: true });
    }
    window.__modularHomeScrollHandler();
  }
