...
(function(){
  ...
  function contribution(r,sign){
    ...
    const signN=sign||1;
    const piece=STATS.pieces.find(p=>String(p.piece)===pieceOf(r));
    ...
    if(t===REPAIR){
      ...
      if(piece){
        piece.requests+=signN;piece.repair+=signN;
        if(s===REPAIR_STAGES[2])piece.repairDone+=signN;
      ...
      if(piece){
        piece.requests+=signN;piece.replacement+=signN;
        if(s===REPLACEMENT_STAGES[2])piece.replacementDone+=signN;
  ...
  function applyChange(c){
    ...
    if(c?.event==="INSERT"){
      ...
      if(isLiveAddition(newR)){seenLiveIds.add(id);contribution(newR,1);}
      return;
    }
    if(c?.event==="DELETE"){
      const id=idOf(oldR);
    ...
    if(c?.event==="UPDATE"){
      // An UPDATE is always a delta: remove old contribution, then add new.
      // This also handles changes of piece/type/stage/status without rebuilding the baseline.
      if(isLiveAddition(oldR)) contribution(oldR,-1);
      if(isLiveAddition(newR)) contribution(newR,1);
      if(isLiveAddition(newR)) seenLiveIds.add(idOf(newR));
    }
  }
