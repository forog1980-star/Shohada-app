...
// نگاشت موقت عناوین قدیمی به عناوین رسمی جدید؛ داده فریز‌شده دستکاری نمی‌شود.
...
<div class="gs-footer-note">این داشبورد فقط داده‌های موجود در بانک را می‌خواند و هیچ ستون یا رکوردی ایجاد یا تغییر نمی‌دهد. عناوین رسمی مراحل در لایه نمایش یکسان شده‌اند.</div>
  ...
  document.getElementById("gs-back")?.addEventListener("click",()=>window.history.state?.page==="statistics"?window.history.back():showHome());
}
...
async function showStatistics(){
  ...
  try{statsRender(statsModel(await statsFetchAll()));}catch(e){app.innerHTML=`<main class="gs-page"><div class="error-message">دریافت آمار انجام نشد.<br><br>${statsEsc(e.message||String(e))}</div></main>`;}
}
